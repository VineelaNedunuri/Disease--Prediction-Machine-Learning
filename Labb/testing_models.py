...
def scale_features(scale_type="standard"):
    ...
    pipe_logistic = Pipeline(
        [("scaler", scaler), ("LR", LogisticRegression(random_state=42))]
    )
    pipe_KNN = Pipeline([("scaler", scaler), ("KNN", KNeighborsClassifier())])
    pipe_tree = Pipeline(
        [("scaling", scaler), ("DT", DecisionTreeClassifier(random_state=42))]
    )
    pipe_forest = Pipeline(
        [("scaling", scaler), ("RF", RandomForestClassifier(random_state=42))]
    )
...
def grid_search(pipeline,param_grid,X_train,y_train,X_val,y_val,dataset_name="dataset1",
    ...
    with open(score_file, "a") as f:
        f.write(
            f"{dataset_name}: F1 score - {f1:.2f}, accuracy-{accuracy * 100:.2f}% \n"
        )
